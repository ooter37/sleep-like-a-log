  ...
  extractor(array) {
    ...
    for (let i = 0; i < splitAsleep.length; i++) {
      obj[splitAsleep[i][1]] = [
        ...
        parseInt(
          moment
            .duration(moment(splitAwake[i][1]).diff(moment(splitAsleep[i][1])))
            .asMinutes()
        ),
      ];
    }
    ...
    const mappedLogs = this.state.logs.map((log, index) => {
      const length = moment
        .utc(
          moment
            .duration(
              moment(log.awake).diff(moment(log.asleep)),
              "milliseconds"
            )
            .asMilliseconds()
        )
          ...
          <td>
            <button
              ...
              onClick={() => {
                if (
                  window.confirm(
                    "Are you sure you wish to delete this log entry?"
                  )
                )
                  this.deleteLog(log.log_id);
              }}
            >
              Delete
            </button>
          </td>
          <td className='log-display-asleep'>
            {moment(log.asleep).format("MMMM Do, h:mm A")}
          </td>
          <td className='log-display-awake'>
            {moment(log.awake).format("MMMM Do, h:mm A")}
          </td>
    ...
    return (
      <div className='log-display-container'>
        <div className='identifier-log-container'>
          <AddLog
            getLogsByBaby={this.getLogsByBaby}
                ...
                <table>
                  <thead className='log-display-container'>
                    <tr className='log-display-labels'>
                      <th
                        className='log-display-asleep sort-asleep'
                        onClick={this.onSort("asleep")}
                      >
                        Asleep
                      ...
                      <th
                        className='log-display-awake sort-awake'
                        onClick={this.onSort("awake")}
                      >
                        Awake
                        <span className={this.setArrow("awake")}></span>
                      </th>
                      <th
                        className='log-display-length sort-length'
                        onClick={this.onSort("length")}
                      >
                        Length
                        <span className={this.setArrow("length")}></span>
                      </th>
                      <th
                        className='log-display-name sort-name'
