  ...
  extractor(array) {
    ...
    for (let i = 0; i < array.length; i++) {
      for (let key in array[i]) {
        if (key === "asleep") {
          splitAsleep.push(array[i][key].split("T"));
        }
      }
    }
    let splitAwake = [];
    for (let i = 0; i < array.length; i++) {
      for (let key in array[i]) {
        if (key === "awake") {
          splitAwake.push(array[i][key].split("T"));
        }
      }
    }
    var subtracted = [];
    for (let i = 0; i < splitAsleep.length; i++) {
      subtracted.push([
        splitAsleep[i][0],
        moment.utc(moment(splitAwake,"DD/MM/YYYY HH:mm:ss").diff(moment(splitAsleep,"DD/MM/YYYY HH:mm:ss"))).format("HH:mm:ss")
      ]);
    }
