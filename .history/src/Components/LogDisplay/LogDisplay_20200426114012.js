...
class LogDisplay extends React.Component {
    ...
    render(){
        ...
        const length = moment.utc(moment.duration(moment(log.awake).diff(moment(log.asleep)), "milliseconds").asMilliseconds()).format("HH:mm")
            return (
                <tr key={log.log_id}>
                    <td className='asleep'>{moment(log.asleep).format('MMMM Do, h:mm A')}</td>
                    <td className='awake' >{moment(log.awake).format('MMMM Do, h:mm A')}</td>
                    <td className='length' >{length}</td>
                    <td className='delete' onClick={() => { if (window.confirm('Are you sure you wish to delete this log entry?')) this.deleteLog(log.log_id) } }
                    >Delete</td>
                </tr>
            )
        })
                    ...
                    </div>
                {
                    ...
                    <div>
                        <table>
                            <thead>
                                <tr>
                                    <th className='asleep' onClick={this.onSort('asleep')}>Asleep</th>
                                    <th className='awake' onClick={this.onSort('awake')}>Awake</th>
                                    <th className='length' onClick={this.onSort('time')}>Time</th>
