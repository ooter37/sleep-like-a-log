...
class LogDisplay extends React.Component {
    ...
    extractor(array) {
        ...
        for (let i = 0; i < splitAsleep.length; i++) {
          ...
          obj[splitAsleep[i][1]] = [splitAsleep[i][0], parseInt((
                moment.utc(moment.duration(moment(splitAwake[i][1]).diff(moment(splitAsleep[i][1])))
              ).asMinutes())
              )
            ]
    ...
    render(){
        ...
        const mappedLogs = this.state.logs.map(log => {
            const sleepTime = moment.utc(moment.duration(moment(log.awake).diff(moment(log.asleep)), "milliseconds").asMilliseconds())
            .format("HH:mm")
            return (
