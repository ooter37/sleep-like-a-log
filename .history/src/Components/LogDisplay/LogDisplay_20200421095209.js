...
export default class LogDisplay extends React.Component {
    ...
    render(){
        const mappedLogs = this.state.logs.map(log => {
            return (
                <div className='log-display' key={log.log_id}>
                    <div className='log-display-awake'>Awake:{moment(log.awake).format('MMMM Do YYYY, h:mm:ss a')}</div>
                    <div className='log-display-asleep'>Asleep:{moment(log.asleep).format('MMMM Do YYYY, h:mm:ss a')}</div>
        ...
        return(
            <div>
                <div>
                    <div className='log-display-awake'>Asleep</div>
                    <div className='log-display-asleep'>Awake</div>
                    <div className='log-display-delete'>Delete</div>
                </div>
                {mappedLogs}
